...
#define HCI_CMD_VNDR_HEARTBEAT 0xFC94
                HC_BT_HDR *p_hb_msg;
                uint8_t *p;
                p_hb_msg = (HC_BT_HDR *) bt_hc_cbacks->alloc(BT_HC_HDR_SIZE + \
                        HCI_CMD_PREAMBLE_SIZE);
                if (p_hb_msg)
                {
                    p_hb_msg->event = MSG_STACK_TO_HC_HCI_CMD;
                    p_hb_msg->offset = 0;
                    p_hb_msg->layer_specific = 0;
                    p_hb_msg->len = HCI_CMD_PREAMBLE_SIZE;
                ...
                {
                    ALOGW("Failed to allocate buffer for heartbeat poll");
                }
            }
        }
#endif
