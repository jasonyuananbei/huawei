...
static void bond_state_changed(bt_status_t status, bt_bdaddr_t *bd_addr, bt_bond_state_t state)
{
    ...
    if (pairing_cb.state == BT_BOND_STATE_BONDING &&
        ...
        BTIF_TRACE_DEBUG3("%s: Bond fail, Retry bonding, times = %d, status = %d", __FUNCTION__, bond_retry_times, status);
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                    BT_PROPERTY_RTKBT_AUTOPAIR_TXPOWER, sizeof(int8_t),
                                    &(p_search_data->inq_res.RTKBT_AUTOPAIR_txpower));
                num_properties++;

                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                    BT_PROPERTY_RTKBT_AUTOPAIR_RCUID, sizeof(uint32_t),
                                    &(p_search_data->inq_res.RTKBT_AUTOPAIR_rcuid));
                num_properties++;

                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                    BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_SUPPORTED, sizeof(int8_t),
                                    &(p_search_data->inq_res.RTKBT_AUTOPAIR_classic_supported));
                num_properties++;
                if(p_search_data->inq_res.RTKBT_AUTOPAIR_classic_supported)
                {
                    BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                        BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_ADDR, sizeof(p_search_data->inq_res.RTKBT_AUTOPAIR_classic_addr),
                                        &(p_search_data->inq_res.RTKBT_AUTOPAIR_classic_addr));
                    num_properties++;
                }
#endif
        ...
        case BTA_DM_DISC_RES_EVT:
        {
                 ...
                 (pairing_cb.sdp_attempts < BTIF_DM_MAX_SDP_ATTEMPTS_AFTER_PAIRING))
            {
                ...
                return;
                 ...
                 (pairing_cb.state == BT_BOND_STATE_BONDING ))
            {
                 BTIF_TRACE_DEBUG1("%s Remote Service SDP fail. Call bond_state_changed_cb BOND_NONE",
                                   __FUNCTION__);
                 pairing_cb.sdp_attempts  = 0;
...
static void btif_dm_generic_evt(UINT16 event, char* p_param)
{
    ...
    switch(event)
    {
        ...
        case BTIF_DM_CB_BOND_FAIL:
            {
                if (pairing_cb.state == BT_BOND_STATE_NONE ) {
                    BTIF_TRACE_ERROR0("already bond none BTIF_DM_CB_BOND_FAIL");
                    return;
                } else if ((bdcmp(((bt_bdaddr_t *)p_param)->address, pairing_cb.bd_addr) != 0)) {
                    BTIF_TRACE_ERROR0("pairing device error BTIF_DM_CB_BOND_FAIL");
                    return;
                }
        ...
        case BTIF_DM_CB_HID_BOND_SUCCESS:
            {
            if (pairing_cb.state == BT_BOND_STATE_NONE ) {
                BTIF_TRACE_ERROR0("already bond none BTIF_DM_CB_HID_BOND_SUCCESS");
                return;
            } else if ((bdcmp(((bt_bdaddr_t *)p_param)->address, pairing_cb.bd_addr) != 0)) {
                BTIF_TRACE_ERROR0("pairing device error BTIF_DM_CB_HID_BOND_SUCCESS");
                return;
            }

                bond_state_changed(BT_STATUS_SUCCESS, (bt_bdaddr_t *)p_param, BT_BOND_STATE_BONDED);
            }
            break;
#endif
        default:
...
#ifdef BLUETOOTH_RTK
    BTA_DmBleSecurityGrant(bd_addr.address,BTA_DM_SEC_PAIR_NOT_SPT);
#else
    HAL_CBACK(bt_hal_cbacks, ssp_request_cb, &bd_addr, &bd_name, cod,
...
void btif_dm_hid_connect_success(BD_ADDR bd_addr)
{
     BTIF_TRACE_ERROR0("btif_dm_hid_connect_success");
     bt_bdaddr_t bdaddr;
     bdcpy(bdaddr.address, bd_addr);
     btif_transfer_context(btif_dm_generic_evt, BTIF_DM_CB_HID_BOND_SUCCESS,
                         (char *)&bdaddr, sizeof(bt_bdaddr_t), NULL);
}
#endif
