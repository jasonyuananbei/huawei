...
void btif_hh_remove_device(bt_bdaddr_t bd_addr)
{
...
#ifdef BLUETOOTH_RTK
    if(BTIF_HH_MAX_ADDED_DEV == i)
    {
        BTIF_TRACE_WARNING1("%s: to call BTA_HhRemoveDevByAddr", __FUNCTION__);
        BTA_HhRemoveDevByAddr(*(BD_ADDR*)&bd_addr);
    }
#endif
    ...
    if(Stop_timer)
    {
        BTIF_TRACE_WARNING1("%s: stop_timer is true", __FUNCTION__);
        btu_stop_timer(&reconnect_timer_ent);
        Stop_timer = FALSE;
    }
#endif
...
bt_status_t btif_hh_virtual_unplug(bt_bdaddr_t *bd_addr)
{
    ...
    {
        BTIF_TRACE_WARNING1("%s: device connected set stop_timer true", __FUNCTION__);
        Stop_timer = TRUE;
    }
#endif
...
#ifdef BLUETOOTH_RTK
                    BTIF_TRACE_WARNING0("BTA_HH_GET_DSCP_EVT: Called btif_dm_hid_connect_success ");
                    btif_dm_hid_connect_success(bda);
#endif
