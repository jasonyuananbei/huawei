...
static bt_status_t btif_rtkbt_autopair_evt(void* p_msg);
#endif
static bt_status_t btif_rtkbt_huawei_rcu_evt(void* p_msg);
            ...
            while((p_msg = GKI_read_mbox(BTU_BTIF_MBOX)) != NULL)
            {
                ...
                switch (p_msg->event)
                {
                    case BT_EVT_CONTEXT_SWITCH_EVT:
                        ...
                        break;
#ifdef BLUETOOTH_RTK_AUTOPAIR
                    case BT_EVT_RTKBT_AUTOPAIR_EVT:
                        btif_rtkbt_autopair_evt(p_msg);
                        break;
#endif
                    case BT_EVT_RTKBT_HUAWEI_RCU_EVT:
                        btif_rtkbt_huawei_rcu_evt(p_msg);
                        break;
                    default:
...
bt_status_t btif_init_bluetooth()
{
 ...
 static bt_status_t btif_rtkbt_huawei_rcu_evt(void* msg)
{
    BT_HDR *p_msg = (BT_HDR *)msg;
    UINT8   *p = (UINT8 *)(p_msg + 1) + p_msg->offset;
    UINT8   len = p_msg->len, i;
    UINT8   sub_event = *p, addr_type;
    p++;
    bt_bdaddr_t bd_addr;
    bt_property_t addr_prop;
    UINT32 id = 0x03;

    BTIF_TRACE_DEBUG1("btif_rtkbt_huawei_rcu_evt, the length is %d: ", len);

    if(len == 8 && sub_event == 0xFF)
    {
        addr_type = *p;
        p++;

        BTIF_TRACE_ERROR0("btif_rtkbt_huawei_rcu_evt: huawei rcu exist!");
        BTIF_TRACE_ERROR0("btif_rtkbt_huawei_rcu_evt: address is!");
        for(i=0; i<len-2; i++)
        {
            BTIF_TRACE_ERROR2("data[%d]=0x%02x", i, p[i]);
        }
        memcpy(&bd_addr, p, BD_ADDR_LEN);

        BTIF_STORAGE_FILL_PROPERTY(&addr_prop, 0xFA, sizeof(id), &id);

        BTIF_TRACE_DEBUG0("btif_rtkbt_huawei_rcu_evt: start to save!");
        btif_storage_set_remote_device_property(&bd_addr, &addr_prop);
        BTIF_TRACE_DEBUG0("btif_rtkbt_huawei_rcu_evt: save success!!");

   }

    //GKI_freebuf (msg);//buffer is freed if btif_task
    return BT_STATUS_SUCCESS;
}

#ifdef BLUETOOTH_RTK_AUTOPAIR
 static bt_status_t btif_rtkbt_autopair_evt(void* msg)
{
    BT_HDR *p_msg = (BT_HDR *)msg;
    UINT8   *p = (UINT8 *)(p_msg + 1) + p_msg->offset;
    UINT8   len = p_msg->len;

    BTIF_TRACE_DEBUG1("btif_rtkbt_autopair_evt, the length is %d: ", len);
    if(rtkbt_autopair_hal_cbacks == NULL)
    {
        BTIF_TRACE_ERROR0("rtkbt_autopair_hal_cbacks is null  ");
    }
    else if(rtkbt_autopair_hal_cbacks->rtkbt_autopair_callback == NULL)
    {
        BTIF_TRACE_ERROR0("rtkbt_autopair_hal_cbacks->rtkbt_autopair_callback is null  ");
    }
    else
    {
        BTIF_TRACE_DEBUG0("rtkbt_autopair_hal_cbacks & rtkbt_autopair_hal_cbacks->rtkbt_autopair_callback is ok  ");
    }
    HAL_CBACK(rtkbt_autopair_hal_cbacks, rtkbt_autopair_callback, (len<64)?len:64, (uint8_t*)p);
