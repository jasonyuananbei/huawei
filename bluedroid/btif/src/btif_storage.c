...
#define BTIF_STORAGE_PATH_BLUEDROID "/swdb/bluedroid"
...
/*BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR Begin*/
#ifdef BLUETOOTH_RTK_AUTOPAIR
#define BTIF_STORAGE_PATH_REMOTE_RTKBT_AUTOPAIR_CLASSIC_SUPPORT "ClassicSup"
#define BTIF_STORAGE_PATH_REMOTE_RTKBT_AUTOPAIR_CLASSIC_ADDR "ClassicAddr"
#define BTIF_STORAGE_PATH_REMOTE_RTKBT_HUAWEI_RCUID "HUAWEIID"
#define BTIF_STORAGE_PATH_REMOTE_RTKBT_AUTOPAIR_RCUID "RcuID"
#endif
/*BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR End*/
...
static int prop2cfg(bt_bdaddr_t *remote_bd_addr, bt_property_t *prop)
{
    ...
    switch(prop->type)
    {
       case BT_PROPERTY_REMOTE_DEVICE_TIMESTAMP:
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
        case BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_ADDR:
        {
            bdstr_t rtk_classic_addr = {0};
            bt_bdaddr_t *classic_addr = (bt_bdaddr_t*)prop->val;
            if(classic_addr) {
                bd2str(classic_addr, &rtk_classic_addr);
            ...
            break;
        case BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_SUPPORTED:
        {
            btif_config_set_int("Remote", bdstr,
        ...
        case 0xFA://BT_PROPERTY_RTKBT_HUAWEI_RCU_ID:
        {
            btif_config_set_int("Remote", bdstr,
                                BTIF_STORAGE_PATH_REMOTE_RTKBT_HUAWEI_RCUID, *(int*)prop->val);
...
static int cfg2prop(bt_bdaddr_t *remote_bd_addr, bt_property_t *prop)
{
    ...
    switch(prop->type)
    {
       case BT_PROPERTY_REMOTE_DEVICE_TIMESTAMP:
        ...
        case BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_ADDR:
        {
            char rtk_classic_addr[18] = {0};
            int size = sizeof(bdstr_t);
            if(btif_config_get_str("Remote", bdstr,
                                    BTIF_STORAGE_PATH_REMOTE_RTKBT_AUTOPAIR_CLASSIC_ADDR, rtk_classic_addr, &size))
            {
                BTIF_TRACE_DEBUG1("cheng_@ bd addr:%s", rtk_classic_addr);
                str2bd(rtk_classic_addr, (bt_bdaddr_t*)prop->val);
                ret = TRUE;
            }
            else
                ret = FALSE;
        ...
        {
            int value = 0;
            if(prop->len >= (char)sizeof(char))
            {
                ret = btif_config_get_int("Remote", bdstr,
                                BTIF_STORAGE_PATH_REMOTE_RTKBT_AUTOPAIR_CLASSIC_SUPPORT, &value);
                if(ret == TRUE)
                    *(char*)prop->val = (char)value;
            }
        }
            break;
...
/*BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR Begin*/
#ifdef BLUETOOTH_RTK_AUTOPAIR
    int8_t rtk_autopair_classic_support;
    bt_bdaddr_t rtk_autopair_classic_addr;
    uint32_t rtk_autopair_rcuid = 0;
    bt_property_t remote_properties[11];
#else   /*BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR modify*/
...
/*BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR Begin*/
#ifdef BLUETOOTH_RTK_AUTOPAIR
            BTIF_STORAGE_GET_REMOTE_PROP(p_remote_addr, BT_PROPERTY_RTKBT_AUTOPAIR_RCUID,
                                         &rtk_autopair_rcuid, sizeof(rtk_autopair_rcuid),
                                         remote_properties[num_props]);
            num_props++;

        /*if RCU id is 0, there is no necessary to acquire
            the value of classic address and classic supported*/
        if(rtk_autopair_rcuid != 0)
        {
            BTIF_STORAGE_GET_REMOTE_PROP(p_remote_addr, BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_ADDR,
                                         &rtk_autopair_classic_addr, sizeof(rtk_autopair_classic_addr),
                                         remote_properties[num_props]);
            num_props++;

            BTIF_STORAGE_GET_REMOTE_PROP(p_remote_addr, BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_SUPPORTED,
                                         &rtk_autopair_classic_support, sizeof(rtk_autopair_classic_support),
                                         remote_properties[num_props]);
            num_props++;
        }
#endif
                                                    ...
                                                    UINT8 ctry_code, UINT16 dl_len, UINT8 *dsc_list)
{
    ...
    if(dl_len > 0)
        btif_config_set("Remote", bdstr, "HidDescriptor", (const char*)dsc_list, dl_len,
                        BTIF_CFG_TYPE_BIN);
    btif_config_flush();
    btif_config_get_int("Remote", bdstr, "DevType", &value);
    BTIF_TRACE_DEBUG0("btif_storage_add_hid_device_info:1111");
    if(value == BT_DEVICE_TYPE_BLE && FALSE == btif_config_get_int("Remote", bdstr,"HUAWEIID", &value))
    {
        btif_config_get_int("Remote", bdstr, "AddrType", &value);
        para[0] = (value & 0x000F);
        //memcpy(para+1, remote_bd_addr->address, 6);
	 for(value = 1; value<=6; value++)
		para[value] = remote_bd_addr->address[6-value];
        BTIF_TRACE_DEBUG0("btif_storage_remove_hid_info: send vendor command to add!");
        BTM_VendorSpecificCommand(0xFC7B, sizeof(para), para, NULL);
    } else
        BTIF_TRACE_DEBUG0("btif_storage_add_hid_device_info:no need to notify fw!");
    return BT_STATUS_SUCCESS;
}
...
bt_status_t btif_storage_load_bonded_hid_info(void)
{
    ...
    BTIF_TRACE_DEBUG0("btif_storage_load_bonded_hid_info, clear whitelist");
    BTM_VendorSpecificCommand(0xFC7D, sizeof(para), para, NULL);
    do
    {
        kpos = btif_config_next_key(kpos, "Remote", kname, &kname_size);
        BTIF_TRACE_DEBUG2("Remote device:%s, size:%d", kname, kname_size);
...
#endif
        {
            ...
            if(btif_config_get_int("Remote", kname,"DevType", &value))
            {
                if(value == BT_DEVICE_TYPE_BLE && FALSE == btif_config_get_int("Remote", kname,"HUAWEIID", &value))
                {
                    BTIF_TRACE_DEBUG0("non huawei BLE HID device found!:");
                    btif_config_get_int("Remote", kname,"AddrType", &value);
                    para[0] = (value & 0x000F);
                    //memcpy(para+1, bd_addr.address, 6);
			for(value = 1; value<=6; value++)
				para[value] = bd_addr.address[6-value];
                    BTM_VendorSpecificCommand(0xFC7B, sizeof(para), para, NULL);
                } else if (FALSE == btif_config_get_int("Remote", kname,"HUAWEIID", &value))
                    BTIF_TRACE_DEBUG1("btif_storage_load_bonded_hid_info, HuaWei_id = %d:", value);
            }
        }
    } while(kpos != -1);
...
{
    BTIF_TRACE_DEBUG0("btif_storage_remove_hid_info");
    char *fname;
    int ret;
    bdstr_t bdstr;
    bd2str(remote_bd_addr, &bdstr);
    ...
    if(value == BT_DEVICE_TYPE_BLE && FALSE == btif_config_get_int("Remote", bdstr,"HUAWEIID", &value))
    {
        ...
        for(value = 1; value<=6; value++)
		para[value] = remote_bd_addr->address[6-value];
        BTIF_TRACE_DEBUG0("btif_storage_remove_hid_info: send vendor command to delete");
        BTM_VendorSpecificCommand(0xFC7C, sizeof(para), para, NULL);
    }
    return BT_STATUS_SUCCESS;
}
