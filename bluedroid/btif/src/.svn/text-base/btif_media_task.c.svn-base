...
#ifdef RTK_A2DP_MEDIA
#define MTU_3DH5 		(1021-8-12-1)  //hci header,l2cap header, media header
#define MTU_2DH5		(679-8-12-1)
#define MTU_3DH3		(552-8-12-1)
#define MTU_2DH3		(367-8-12-1)
#endif
/*
...
#define MAX_OUTPUT_A2DP_FRAME_QUEUE_SZ 6
...
#ifdef RTK_A2DP_MEDIA
void rtk_update_mtu_timer(void)
{
	UINT32 output_frame_size;
	UINT8 num_frame;
	UINT32 pcm_bytes_per_frame;
	UINT32 pcm_bytes_counter;
	UINT8 edr = btif_av_get_peer_edr();
	APPL_TRACE_DEBUG1("realtek, edr is 0x%x", edr);

	if((edr & BTA_AV_EDR_3DH5) && (btif_media_cb.TxAaMtuSize > MTU_3DH5))
		btif_media_cb.TxAaMtuSize = MTU_3DH5;
	else if( !(edr & BTA_AV_EDR_3DH5) && (edr & BTA_AV_EDR_2DH5) && (btif_media_cb.TxAaMtuSize > MTU_2DH5) )
		btif_media_cb.TxAaMtuSize = MTU_2DH5;
	else if( !(edr & BTA_AV_EDR_3DH5) && !(edr & BTA_AV_EDR_2DH5) 
				&& (edr & BTA_AV_EDR_3DH3) && (btif_media_cb.TxAaMtuSize > MTU_3DH3))
		btif_media_cb.TxAaMtuSize = MTU_3DH3;
	else if(!(edr & BTA_AV_EDR_3DH5) && !(edr & BTA_AV_EDR_2DH5) && !(edr & BTA_AV_EDR_3DH3) 
				&& (edr & BTA_AV_EDR_2DH3) && (btif_media_cb.TxAaMtuSize > MTU_2DH3))
		btif_media_cb.TxAaMtuSize = MTU_2DH3;
	APPL_TRACE_DEBUG1("realtek, btif_media_cb.TxAaMtuSize is %d", btif_media_cb.TxAaMtuSize);

	output_frame_size = 4 + (4 * btif_media_cb.encoder.s16NumOfSubBands * btif_media_cb.encoder.s16NumOfChannels)/8
						+(btif_media_cb.encoder.s16NumOfSubBands + btif_media_cb.encoder.s16NumOfBlocks * btif_media_cb.encoder.s16BitPool)/8;
	APPL_TRACE_DEBUG1("realtek, output_frame_size is %d", output_frame_size);

	num_frame = btif_media_cb.TxAaMtuSize/output_frame_size;
	APPL_TRACE_DEBUG1("realtek, num_frame is %d", num_frame);

	pcm_bytes_per_frame = btif_media_cb.encoder.s16NumOfSubBands * btif_media_cb.encoder.s16NumOfBlocks *
                             btif_media_cb.media_feeding.cfg.pcm.num_channel * btif_media_cb.media_feeding.cfg.pcm.bit_per_sample / 8;
	APPL_TRACE_DEBUG1("realtek, pcm_bytes_per_frame is %d", pcm_bytes_per_frame);

    pcm_bytes_counter = pcm_bytes_per_frame * num_frame;

	rtk_tick = pcm_bytes_counter * 1000 /
		(btif_media_cb.media_feeding.cfg.pcm.sampling_freq *
		 btif_media_cb.media_feeding.cfg.pcm.bit_per_sample / 8 *
		 btif_media_cb.media_feeding.cfg.pcm.num_channel);
	APPL_TRACE_DEBUG1("realtek, rtk_tick is %d", rtk_tick);
}
#endif
/*******************************************************************************
 **
...
#ifdef RTK_A2DP_MEDIA
	APPL_TRACE_EVENT2("starting rtk timer %d ticks (%d)",
			  GKI_MS_TO_TICKS(rtk_tick), TICKS_PER_SEC);

	GKI_start_timer(BTIF_MEDIA_AA_TASK_TIMER_ID, GKI_MS_TO_TICKS(rtk_tick), TRUE);
#else
