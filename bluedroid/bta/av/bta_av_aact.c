...
void bta_av_str_opened (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
...

    {
        ...
        if( NULL != (p = BTM_ReadRemoteFeatures(p_scb->peer_addr)))
        {
			...
			if(HCI_EDR_ACL_3MPS_SUPPORTED(p) && HCI_3_SLOT_EDR_ACL_SUPPORTED(p))
				open.edr |= BTA_AV_EDR_3DH3;

			if(HCI_EDR_ACL_3MPS_SUPPORTED(p) && HCI_5_SLOT_EDR_ACL_SUPPORTED(p))
				open.edr |= BTA_AV_EDR_3DH5;

			if(HCI_EDR_ACL_2MPS_SUPPORTED(p) && HCI_3_SLOT_EDR_ACL_SUPPORTED(p))
				open.edr |= BTA_AV_EDR_2DH3;

			if(HCI_EDR_ACL_2MPS_SUPPORTED(p) && HCI_5_SLOT_EDR_ACL_SUPPORTED(p))
				open.edr |= BTA_AV_EDR_2DH5;
        }
#if( defined BTA_AR_INCLUDED ) && (BTA_AR_INCLUDED == TRUE)
