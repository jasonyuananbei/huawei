...
static void bta_dm_sys_hw_cback( tBTA_SYS_HW_EVT status )
{
    ...
    if( status == BTA_SYS_HW_ON_EVT )
    {
        ...
        * We need to revisit when this platform has more than one BLuetooth H/W chip */
        ...
        BTM_VendorSpecificCommand(0x77, 7, "\x01\x00\x00\x00\x00\x00\x00", NULL);
#endif
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
    BTM_VendorSpecificCommand(0x77, 7, "\xff\x00\x00\x00\x00\x00\x00", NULL);
#endif
...
#ifdef BLUETOOTH_RTK
        if(!strncmp(remote_device_name, "MSI FS300", 10)&&num_uuids == 0)
        {
            BTIF_TRACE_DEBUG1("%s: Use specific uuid_list for MSI FS300", __FUNCTION__);
            UINT8 i;
            num_uuids = 4;
            bta_dm_search_cb.services_to_search = 0;
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
    memcpy(&(result.inq_res.RTKBT_AUTOPAIR_classic_addr), &(p_inq->RTKBT_AUTOPAIR_classic_addr), sizeof(p_inq->RTKBT_AUTOPAIR_classic_addr));
    result.inq_res.RTKBT_AUTOPAIR_rcuid = p_inq->RTKBT_AUTOPAIR_rcuid;
    result.inq_res.RTKBT_AUTOPAIR_txpower = p_inq->RTKBT_AUTOPAIR_txpower;
    result.inq_res.RTKBT_AUTOPAIR_classic_supported = p_inq->RTKBT_AUTOPAIR_classic_supported;
    APPL_TRACE_DEBUG6("bta_dm_inq_results_cb:rcuid=%d txpower=%d classic_supported=%d classic_addr=%02x:**:%02x:**:%02x:**",result.inq_res.RTKBT_AUTOPAIR_rcuid,result.inq_res.RTKBT_AUTOPAIR_txpower,result.inq_res.RTKBT_AUTOPAIR_classic_supported,result.inq_res.RTKBT_AUTOPAIR_classic_addr.address[0],result.inq_res.RTKBT_AUTOPAIR_classic_addr.address[2],result.inq_res.RTKBT_AUTOPAIR_classic_addr.address[4]);
#endif
#endif
