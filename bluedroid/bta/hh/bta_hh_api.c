...
#ifdef BLUETOOTH_RTK
void BTA_HhRemoveDevByAddr(BD_ADDR bda)
{
    tBTA_HH_API_RM_DEV *p_buf;

    p_buf = (tBTA_HH_API_CONN *)GKI_getbuf((UINT16)sizeof(tBTA_HH_API_RM_DEV));

    if (p_buf!= NULL)
    {
        memset((void *)p_buf, 0, sizeof(tBTA_HH_API_CONN));
        p_buf->hdr.event            = BTA_HH_API_RM_DEV_EVT;
        p_buf->hdr.layer_specific   = BTA_HH_INVALID_HANDLE;
        bdcpy(p_buf->bd_addr, bda);

        bta_sys_sendmsg((void *)p_buf);
    }
    else
    {
        APPL_TRACE_ERROR0("No resource to send remove device.");
    }
}
#endif
