...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    if (p_cb->status == BTA_HH_OK)
    {
        ...
        else
        {
            ...
            APPL_TRACE_DEBUG2("%s, + p_dev_cb->disc_flag = %d", __FUNCTION__, p_cb->disc_flag );
            p_cb->disc_flag |= BTA_HH_LE_SEC_CMPL;
            APPL_TRACE_DEBUG2("%s, ++ p_dev_cb->disc_flag = %d", __FUNCTION__, p_cb->disc_flag );
            if(!(p_cb->disc_flag & BTA_HH_LE_MTU_CMPL))
                return;
#endif
            APPL_TRACE_DEBUG2("%s, p_cb->total_srvc = %d", __FUNCTION__, p_cb->total_srvc );
            if(p_cb->total_srvc >0)
                p_cb->total_srvc = 0;
#endif
...
/*BOARD_HAVE_BLUETOOTH_RTK begin*/
#ifdef BLUETOOTH_RTK
        case UUID_SERVCLASS_DEVICE_INFO :   /* Device information service */
            bta_hh_le_search_dis_chars(p_dev_cb);
            break;
#endif
/*BOARD_HAVE_BLUETOOTH_RTK end*/
        }
...
/*BOARD_HAVE_BLUETOOTH_RTK begin*/
            ...
            case GATT_UUID_PNP_ID:
            {
                ...
                p_dis_value.attr_mask |= DIS_ATTR_PNP_ID_BIT;
                STREAM_TO_UINT8(p_dis_value.pnp_id.vendor_id_src, pp);
                STREAM_TO_UINT16(p_dis_value.pnp_id.vendor_id, pp);
                STREAM_TO_UINT16(p_dis_value.pnp_id.product_id, pp);
                STREAM_TO_UINT16(p_dis_value.pnp_id.product_version, pp);
                bta_hh_le_dis_cback(p_dev_cb->addr, &p_dis_value);
                return;
            }
#endif
/*BOARD_HAVE_BLUETOOTH_RTK end*/
...
static void bta_hh_le_search_dis(tBTA_HH_DEV_CB *p_cb)
{
    ...
    p_cb->disc_active  |= BTA_HH_LE_DISC_DIS;
    /* start  service discovery for Device information service */
    pri_srvc.len        = LEN_UUID_16;
    pri_srvc.uu.uuid16  = UUID_SERVCLASS_DEVICE_INFO;
...
static void bta_hh_le_search_dis_chars(tBTA_HH_DEV_CB *p_cb)
{
    ...
    bta_hh_le_fill_16bits_srvc_id(TRUE, 0, UUID_SERVCLASS_DEVICE_INFO , &srvc_id);
    APPL_TRACE_DEBUG1("%s", __FUNCTION__);

    char_cond.len   = LEN_UUID_16;
    char_cond.uu.uuid16 = GATT_UUID_PNP_ID;
