...
int rtk_alloc_timer(int index);
int  rtk_start_timer(int index);
int  rtk_stop_timer(int index);
int  rtk_free_timer(int index);
/*rtk add timer func end*/

/*add for lost event*/
#define TOTAL_MAX_ENTRIES 6 //max 6 connection handles

typedef struct
{
        UINT16 conn_handle;
        UINT16 signo;
	timer_t timer_retrans;
}RTK_TIMER;

RTK_TIMER rtk_timer[TOTAL_MAX_ENTRIES];

#define RETRANS_TIMEOUT_VALUE 3000 //3s check one time
#ifdef BLUETOOTH_RTK_AUTOPAIR
#include "rtk_autopair.h"
extern UINT32 remote_controller_id;
static void rtkbt_autopair_send_message(short evt, UINT8 evt_opcode, UINT8 *p, UINT8 evt_len) ;
static UINT32 rtkbt_autopair_get_rcuid(UINT8 *p, UINT16 evt_len) ;
#endif
...
#define HW_RCU_NUM 10
static HW_RCU_ADDR hw_rcu_addr[HW_RCU_NUM];

static void init()
{
    int i;
    for(i=0; i<HW_RCU_NUM; i++)
    {
        bdsetany(hw_rcu_addr[i].bda) ;
        hw_rcu_addr[i].used = FALSE;
    }
}

static BOOLEAN look_up_hw_rcu(BD_ADDR bda)
{
    BOOLEAN ret = FALSE;
    int i;
    for(i=0; i<HW_RCU_NUM; i++)
    {
        if(0 == bdcmp(bda, hw_rcu_addr[i].bda) && hw_rcu_addr[i].used == TRUE)
            return TRUE;
    }

    for(i=0; i<HW_RCU_NUM; i++)
    {
        if((0 == bdcmpany(hw_rcu_addr[i].bda)) || (hw_rcu_addr[i].used == FALSE))
        {
            bdcpy(hw_rcu_addr[i].bda, bda) ;
            hw_rcu_addr[i].used = TRUE;
            return ret;
        }
    }
    return ret;
...
static void btu_hcif_check_hw_rcu(UINT8 *p)
{
    BD_ADDR             bda;
    UINT8               evt_type = 0;
    UINT8               addr_type = 0, len = 0;
    UINT8 		*manu_data;

    UINT8 event_len = 6, sub_event = 0xFF;
    UINT8 *event_data;
    BT_HDR *p_msg;
    UINT8 *pp;

    event_data = (UINT8 *)GKI_getbuf(event_len);
    ...
    if(TRUE == look_up_hw_rcu(bda))
    {
        BTM_TRACE_DEBUG0("btu_hcif_check_hw_rcu: This address arleady processed!!");
        return;
    } else
        BTM_TRACE_DEBUG0("btu_hcif_check_hw_rcu: This address is the new one!!");

     if(NULL != (manu_data = BTM_CheckAdvData( p, BTM_BLE_AD_TYPE_MANU, &len)))
     {
	    BTM_TRACE_DEBUG0("BTM_BLE_AD_TYPE_MANU exit!!");
	    if(manu_data[0] == 0x5d && manu_data[1] == 0x00 && manu_data[2] == 0x04 && manu_data[3] == 0x00)
	    {
		   BTM_TRACE_DEBUG0("HUAWEI exit!!");
		   //event_data[0] = sub_event;
		   memcpy(event_data, bda, BD_ADDR_LEN);
		   //btm_vendor_specific_evt (event_data, event_len);


                if ((p_msg = (BT_HDR *)GKI_getbuf(BT_HDR_SIZE + event_len + sizeof(UINT8) + sizeof(UINT8))) != NULL)
                 {
                        p_msg->event = BT_EVT_RTKBT_HUAWEI_RCU_EVT;
                        p_msg->offset = 0;
                        p_msg->len = 2 + event_len;
                        pp = (UINT8 *)(p_msg + 1);
                        UINT8_TO_STREAM (pp, sub_event);//event code
                        UINT8_TO_STREAM (pp, addr_type); //addr_type
                        if(event_len > 0)
                            ARRAY_TO_STREAM(pp, event_data, event_len);//event data
                        GKI_send_msg (BTIF_TASK, BTU_BTIF_MBOX, p_msg);
                }
		}
       }

     GKI_freebuf(event_data);
}
...
#ifdef BLUETOOTH_RTK
            rtkbt_subevent_code = *p;
            switch(rtkbt_subevent_code){


#ifdef BLUETOOTH_RTK_AUTOPAIR
                case HCI_RTKBT_AUTOPAIR_EVT:    //0x30
                    BT_TRACE_0(TRACE_LAYER_HCI, TRACE_TYPE_EVENT, "autopair vendor event");
                    if(rtkbt_autopair_get_rcuid(p,hci_evt_len) > 0){
                        BT_TRACE_0(TRACE_LAYER_HCI, TRACE_TYPE_EVENT, "autopair vendor event start discovery");
                        rtkbt_autopair_send_message(BT_EVT_RTKBT_AUTOPAIR_EVT, rtkbt_subevent_code, p, hci_evt_len);       // For test
                        }
                    btu_ble_process_adv_pkt(p+1, hci_evt_len-1);
                    break;
#endif
                default:
...
static void rtkbt_autopair_send_message(short evt, UINT8 evt_opcode, UINT8 *p, UINT8 evt_len)
{
    ...
    pp = p;
    STREAM_TO_UINT8  (ble_sub_code, pp);

    BT_TRACE_1 (TRACE_LAYER_HCI, TRACE_TYPE_WARNING,"rtkbt_autopair_send_message:ble_sub_code=%d",ble_sub_code);
    ...
    if ((p_msg = (BT_HDR *)GKI_getbuf(BT_HDR_SIZE + evt_len + sizeof(UINT8) + sizeof(UINT8))) != NULL)
     {
            ...
            p_msg->len = 2 + evt_len;
            pp = (UINT8 *)(p_msg + 1);
            UINT8_TO_STREAM (pp, evt_opcode);//event code
            UINT8_TO_STREAM (pp, evt_len); //event length
            if(evt_len > 0)
                ARRAY_TO_STREAM(pp, p, evt_len);//event data
            GKI_send_msg (BTIF_TASK, BTU_BTIF_MBOX, p_msg);
      }
}
...
static UINT32 rtkbt_autopair_get_rcuid(UINT8 * p,UINT16 evt_len)
{
    ...
    UINT16 adv_len = 0;
    UINT16 len = 0;
    UINT8  classic_supported;
    bt_bdaddr_t classic_addr;
    UINT32 remote_controller_rcuid = 0;
    UINT8   num, i,j;

    pp++;
    num = *pp;

    BT_TRACE_2 (TRACE_LAYER_HCI, TRACE_TYPE_WARNING,"evt_len = %d, num = %d",evt_len,num);
    for(i = 0; i < num; i++)
    {
        pp = pp + 9;
        adv_len = *pp;
        BT_TRACE_1 (TRACE_LAYER_HCI, TRACE_TYPE_WARNING,"adv_len = %d",adv_len);
        pp++;
        while(adv_len != 0)
        {
            len = *pp;
            pp++;
            if(*pp == BTM_BLE_AD_TYPE_MANU)
            {
                RTKBT_AUTOPAIR_CheckMSD(pp+1, len, &remote_controller_rcuid, &classic_supported, &classic_addr);
                BT_TRACE_1 (TRACE_LAYER_HCI, TRACE_TYPE_WARNING,"remote_controller_rcuid = %d",remote_controller_rcuid);
            }
            pp += len;
            adv_len -= (len+1);
        }
    }
    return remote_controller_rcuid;
}
#endif
...
/*add for lost event*/
    ...
    {
	UINT8 num_index;
	for(num_index = 0; num_index < TOTAL_MAX_ENTRIES; num_index++)
	{
	    if(rtk_timer[num_index].conn_handle == 0xFFFF)
	    {
		rtk_timer[num_index].conn_handle = handle;
		//total_number[num_index].total_complete_num = 0;
		break;
	    }
	}
	if(num_index != TOTAL_MAX_ENTRIES)
	{
	    rtk_alloc_timer(num_index);
	    rtk_start_timer(num_index);
	}
...
static void btu_hcif_disconnection_comp_evt (UINT8 *p, UINT16 evt_len)
{
	...
	for(num_index = 0; num_index < TOTAL_MAX_ENTRIES; num_index++)
	{
	    if(rtk_timer[num_index].conn_handle == handle)
	    {
		rtk_timer[num_index].conn_handle = 0xFFFF;
		break;
	    }
	}
	...
	if(num_index != TOTAL_MAX_ENTRIES)
	{
	    rtk_stop_timer(num_index);
	    rtk_free_timer(num_index);
	}
    }
...
/* BLE Commands */
	...
	{
		btm_ble_cancel_conn_complete(p);
	}
...
static void btu_hcif_command_complete_evt (UINT8 controller_id, UINT8 *p, UINT16 evt_len)
{
	...
	{
	    rtk_timer[num_index].conn_handle = 0xFFFF;
	    rtk_timer[num_index].signo = SIGRTMAX - num_index;
	}
    }
...
static int rtk_generate_num_of_comp(UINT16 handle, int not_acked)
{
    ALOGE("rtk generate num of comp");
    UINT8 *p = GKI_getbuf(5 * sizeof(UINT8));
    
    if(p != NULL)
    {
	p[0] = 1; // num of handles
	p[1] = (handle & 0xFF);
	p[2] = ((handle >> 8) & 0xFF); //conn handle
	p[3] = (not_acked & 0xFF);
	p[4] = ((not_acked >> 8) & 0xFF); //num_sent;
...
static void rtk_timeout_handler(int signo, siginfo_t * info, void *context)
{
    ALOGE("rtk timeout handle");
	...
	for(num_index = 0; num_index < TOTAL_MAX_ENTRIES; num_index++)
	{
	    if(rtk_timer[num_index].signo == signo)
	    {
		handle =  rtk_timer[num_index].conn_handle;
		break;
	    }
	}

    if((num_index != TOTAL_MAX_ENTRIES) && (handle != 0xffff))
    {
	p_lcb = l2cu_find_lcb_by_handle (handle);
	if(p_lcb)
	{
	    if((p_lcb->sent_not_acked > 0) && (p_lcb->get_num_of_complete == 0))
	    {
		ALOGE("check no acked, handle is %x", handle);
		rtk_generate_num_of_comp(handle, p_lcb->sent_not_acked);
		rtk_start_timer(num_index);
	    }else
	    {
		p_lcb->get_num_of_complete = 0;
		rtk_start_timer(num_index);
		ALOGE("anything is ok, nothing to do");
	    }
	}
    }
}
...
int rtk_alloc_timer(int index)
{
    ...
    sigact.sa_sigaction = rtk_timeout_handler;

    // Set up sigaction to catch signal first timer
    if (sigaction(rtk_timer[index].signo, &sigact, NULL) == -1)
    {
   //     LogMsg("sigaction failed");
    ...
    rtk_timer[index].timer_retrans = OsAllocateTimer(rtk_timer[index].signo);
...
int  rtk_start_timer(int index)
{
    ...
    return OsStartTimer(rtk_timer[index].timer_retrans, RETRANS_TIMEOUT_VALUE, 0);
}

int rtk_stop_timer(int index)
{
    return OsStopTimer(rtk_timer[index].timer_retrans);
}

int rtk_free_timer(int index)
{
    return OsFreeTimer(rtk_timer[index].timer_retrans);
}
