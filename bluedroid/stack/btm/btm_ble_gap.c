...
BOOLEAN btm_ble_update_inq_result(tINQ_DB_ENT *p_i, UINT8 addr_type, UINT8 evt_type, UINT8 *p)
{
    ...
    UINT8               *p_BLUETOOTH_RTK_AUTOPAIR_eir;
    UINT8               p_BLUETOOTH_RTK_AUTOPAIR_ClassicSupported;
#endif
    ...
    if (p_le_inq_cb->adv_len != 0)
    {
        if ((p_flag = BTM_CheckAdvData(p_le_inq_cb->adv_data_cache, BTM_BLE_AD_TYPE_FLAG, &len)) != NULL)
            p_cur->flag = * p_flag;
#ifdef BLUETOOTH_RTK_AUTOPAIR
        if ((p_flag = BTM_CheckAdvData(p_le_inq_cb->adv_data_cache, BTM_BLE_AD_TYPE_TX_PWR, &len)) != NULL)
            p_cur->RTKBT_AUTOPAIR_txpower = * p_flag;
        else
            p_cur->RTKBT_AUTOPAIR_txpower = 0;

        if ((p_BLUETOOTH_RTK_AUTOPAIR_eir = BTM_CheckAdvData(p_le_inq_cb->adv_data_cache, BTM_BLE_AD_TYPE_MANU, &len)) != NULL)
        {
            if(RTKBT_AUTOPAIR_CheckMSD(p_BLUETOOTH_RTK_AUTOPAIR_eir, len, &(p_cur->RTKBT_AUTOPAIR_rcuid), &(p_cur->RTKBT_AUTOPAIR_classic_supported), &(p_cur->RTKBT_AUTOPAIR_classic_addr)) != 0)
                to_report = FALSE;
            else{
                BTM_TRACE_ERROR4("to_report=%d rcuid=%d txpower=%d classic_supported=%d", (int)to_report, p_cur->RTKBT_AUTOPAIR_rcuid,p_cur->RTKBT_AUTOPAIR_txpower,p_cur->RTKBT_AUTOPAIR_classic_supported);
                BTM_TRACE_ERROR6("ble_addr: %02x:%02x:%02x:%02x:%02x:%02x", p_cur->RTKBT_AUTOPAIR_classic_addr.address[0],p_cur->RTKBT_AUTOPAIR_classic_addr.address[1], p_cur->RTKBT_AUTOPAIR_classic_addr.address[2],p_cur->RTKBT_AUTOPAIR_classic_addr.address[3], p_cur->RTKBT_AUTOPAIR_classic_addr.address[4],p_cur->RTKBT_AUTOPAIR_classic_addr.address[5]);
                BTM_TRACE_ERROR6("classic_addr: %02x:%02x:%02x:%02x:%02x:%02x", p_cur->remote_bd_addr[0],p_cur->remote_bd_addr[1], p_cur->remote_bd_addr[2],p_cur->remote_bd_addr[3], p_cur->remote_bd_addr[4],p_cur->remote_bd_addr[5]);
            }
        }
#endif
    }
...
#ifdef BLUETOOTH_RTK
int btm_ble_check_cod(UINT8* data, UINT8* flag,int *cod)
{
    UINT8 data_len = *data;
    UINT8* p;
    ...
    BTM_TRACE_DEBUG1("data_len = %d", data_len);
    for(i = 0; i<data_len; i++)
        BTM_TRACE_DEBUG2("data[%d] = %x", i, data[i]);
...
#ifdef BLUETOOTH_RTK
        res = btm_ble_check_cod(pp , &flag, &cod);
        if(res & 0x2)
        {
...
void btm_ble_update_mode_operation(UINT8 link_role, BD_ADDR bd_addr, BOOLEAN conn_cancel)
{
    ...
    if (btm_ble_get_conn_st() == BLE_CONN_IDLE)
    {
        ...
        {
#ifdef BLUETOOTH_RTK
            btu_start_timer (&reconnect_timer_ent, BTU_TTYPE_BLE_RECONNECT,BTU_TTYPE_BLE_RECONNECT_TIMEOUT);
            BTM_TRACE_DEBUG0("start reconnect timer");
#else
            btm_ble_resume_bg_conn();
#endif
        }
        ...
        BTM_TRACE_DEBUG0("ERROR:conn state is not idle!!");
#endif
}
