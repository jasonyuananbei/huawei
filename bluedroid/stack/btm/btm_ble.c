                             ...
                             tBLE_ADDR_TYPE addr_type)
{
    ...
    if(!p_info)
    {
        BTM_TRACE_DEBUG0("new p_info");
        tINQ_DB_ENT *p_db_ent = btm_inq_db_new (bd_addr);
        p_info = BTM_InqDbRead(bd_addr);
    }
#endif
...
void btm_ble_cancel_conn_complete(UINT8 *p)
{
    UINT8   status;

    STREAM_TO_UINT8(status, p);
	if(btm_cb.ble_ctr_cb.conn_state != BLE_CONN_IDLE)
	{
    ...
    btm_ble_update_mode_operation(HCI_ROLE_UNKNOWN, 0, TRUE);
}
